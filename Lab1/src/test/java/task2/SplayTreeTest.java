...
public class SplayTreeTest {
    ...
    private static Stream<Arguments> insertSequenceProvider() {
        return Stream.of(
                Arguments.of(
                        new int[]{50, 60, 70},
                        new String[]{
                ...
                Arguments.of(
                        new int[]{50, 30, 40, 20, 35},
                        new String[]{
                                "50",
                                "30(null,50)",
                                "40(30,50)",
                                "20(null,30(null,40(null,50)))",
                                "35(30(20,null),40(null,50))"
                        }
                )
        );
    }
    ...
    @ParameterizedTest
    @MethodSource("insertSequenceProvider")
    void testInsertSequence(int[] valuesToInsert, String[] expectedStates) {
        SplayTree tree = new SplayTree();
        ...
        for (int i = 0; i < valuesToInsert.length; i++) {
            ...
            // проверяем состояние после каждой вставки
            String actualState = treeToString(tree);
            assertEquals(expectedStates[i], actualState,
                    "State mismatch after inserting " + value + " at step " + (i+1));
        }
    }

    private String treeToString(SplayTree tree) {
    ...
    private String nodeToString(Node node) {
        ...
        String left = nodeToString(node.getLeft());
        String right = nodeToString(node.getRight());

        // Если оба потомка null, возвращаем просто значение
        if (left.equals("null") && right.equals("null")) {
            return String.valueOf(node.getKey());
        }

        // Всегда показываем оба потомка в порядке: левый, правый
